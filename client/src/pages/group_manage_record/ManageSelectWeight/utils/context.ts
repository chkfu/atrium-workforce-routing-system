...
interface SelectWeightContextType {
  //  1. GET
...
export const SelectWeightContext = createContext<SelectWeightContextType | undefined>(undefined);

export const useSelectWeightContext = () => {
  const context = useContext(SelectWeightContext);
  if (!context) {
    throw new Error('[ManageSelectWeight error: missing useContext at context provider.');
  }
  return context;
};
