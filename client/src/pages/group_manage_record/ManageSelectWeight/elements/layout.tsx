...
export function PanelFromContainer(): JSX.Element {
  const { isInitialised, setIsInitialised } = useSelectWeightContext();
  useEffect(() => {
...
export function TableSection(): JSX.Element {
  const { selectedWeight } = useSelectWeightContext();
  const select_weight = useSelector((state: RootState) => state.select_weight.value);
  if (select_weight.length === 0) {
    return (
      <div className="pt-2 w-full">
        <div className="p-4 text-gray-500">No Select Weight Strategy found</div>
      </div>
    );
  }
  return (
    <div className="pt-2 flex flex-col flex-1 overflow-hidden">
      <p
        className={`mb-2 text-sm text-gray-500 transition-opacity duration-500 ease-in-out
          ${selectedWeight.length === 0 ? 'invisible' : 'visible'}
          ${selectedWeight.length === 0 ? 'opacity-0' : 'opacity-100'}`}
      >
        Selection: {selectedWeight.length}{' '}
        {selectedWeight.length === 1 ? 'strategy' : 'strategies'} selected
      </p>
      <div className="h-84 overflow-x-auto overflow-y-auto">
