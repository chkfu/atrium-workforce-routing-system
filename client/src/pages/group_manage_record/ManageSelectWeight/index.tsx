...
export default function ManageSelectWeight(): JSX.Element {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        const SelectWeightEls = res.data.data.result;
        const totalPages = res.data.data.total_pages;
        dispatch(setSelectWeight(SelectWeightEls));
        setTotalPage(totalPages);
        setIsGetting(false);
      })
      .catch((err: any) => {
        console.error('[ManageSelectWeight] error: fetching Select Weight:', err);
        // learnt: state change for re-render, as throw error did not trigger
        dispatch(setSelectWeight([]));
        setGetError(err.message || '[ManageSelectWeight] error: Failed to load Select Weight');
        setIsGetting(false);
      });
