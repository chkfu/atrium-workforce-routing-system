...
export const ButtonDelete = (): JSX.Element => {
  const dispatch = useDispatch();
  const { selectedCandidates, setSelectedCandidates, isDeleting, setIsDeleting } =
    useCandidateContext();
  return (
    <ButtonConfirm
      label={isDeleting ? 'Loading...' : 'Delete'}
      onClick={() =>
        handle_delete_submit(selectedCandidates, setIsDeleting, setSelectedCandidates, dispatch)
      }
      style={{
        backgroundColor: COLORS.light_gray,
        color: COLORS.dark_teal,
